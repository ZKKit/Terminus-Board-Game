...
class TerminusGame:
    def __init__(self, time_limit, increment):
        ...
        self.win_reason = ""
        self.flipped = False
        # Mutual draw checkboxes: {B_PAWN: bool, W_PAWN: bool}
        self.draw_agreement = {B_PAWN: False, W_PAWN: False}
    ...
    def make_move(self, r1, c1, r2, c2, move_type):
        ...
        # Modified Log Format: Turn X: Color Origin > Target
        self.move_log.append(f"Turn {self.turn_count}: {p_name} {cols[c1]}{12-r1} > {cols[c2]}{12-r2}")
        ...
        # Reset draw agreement if a move is made
        self.draw_agreement = {B_PAWN: False, W_PAWN: False}
        
        if self.current_player == W_PAWN: self.turn_count += 1
...
def main():
    ...
    while True: 
        ...
        while menu_active:
            ...
            # --- CUSTOM UI LABELS ---
            screen.blit(f_xs.render("mins", True, COORD_COLOR), (min_rect.x, min_rect.bottom + 5))
            screen.blit(f_xs.render("inc (secs)", True, COORD_COLOR), (inc_rect.x, inc_rect.bottom + 5))
        ...
        t_limit = int(custom_min or 5)*60 if sel_time == 4 else time_opts[sel_time][1]
        t_inc = int(custom_inc or 0) if sel_time == 4 else time_opts[sel_time][2]
        ...
        while game_running:
            ...
            for r in range(ROWS):
                for c in range(COLS):
                    ...
                    if rect.collidepoint(m_pos) and not confirm_resign_who and not confirm_exit_to_menu:
                        s = pygame.Surface((SQ_SIZE, SQ_SIZE), pygame.SRCALPHA); s.fill(HOVER_COLOR); screen.blit(s, rect.topleft)
                    ...
                    if state != EMPTY: draw_piece(screen, state, rect.centerx, rect.centery)
            ...
            # Turn/Win Status
            stat_rect = pygame.Rect(sidebar_x + 20, OFFSET + 120, 300, 60)
            pygame.draw.rect(screen, (45, 50, 60), stat_rect, border_radius=10)
            if game.winner:
                res_txt = f"{game.winner.upper()} WINS" if game.winner != "Draw" else "DRAW"
                screen.blit(f_m.render(res_txt, True, (120, 240, 140)), (stat_rect.x+20, stat_rect.y+15))
            else:
                curr_turn = "BLACK'S TURN" if game.current_player == B_PAWN else "WHITE'S TURN"
                screen.blit(f_m.render(curr_turn, True, TEXT_COLOR), (stat_rect.x+20, stat_rect.y+15))

            # --- MUTUAL DRAW AGREEMENT SYSTEM ---
            draw_panel = pygame.Rect(sidebar_x + 20, OFFSET + 190, 300, 80)
            pygame.draw.rect(screen, (40, 45, 55), draw_panel, border_radius=10)
            screen.blit(f_xs.render("DRAW AGREEMENT", True, COORD_COLOR), (draw_panel.x + 10, draw_panel.y + 5))
            
            cb_w = pygame.Rect(draw_panel.x + 20, draw_panel.y + 35, 20, 20)
            cb_b = pygame.Rect(draw_panel.x + 160, draw_panel.y + 35, 20, 20)
            
            for cb, player, label in [(cb_w, W_PAWN, "WHITE"), (cb_b, B_PAWN, "BLACK")]:
                pygame.draw.rect(screen, (60, 65, 75), cb, border_radius=4)
                if game.draw_agreement[player]:
                    pygame.draw.rect(screen, (80, 200, 120), cb.inflate(-6, -6), border_radius=2)
                screen.blit(f_s.render(label, True, TEXT_COLOR), (cb.right + 10, cb.y))

            by = OFFSET + 290
            btn_res_w = Button(sidebar_x+20, by, 300, 40, "WHITE RESIGN", f_s, (70, 75, 85))
            btn_res_b = Button(sidebar_x+20, by+45, 300, 40, "BLACK RESIGN", f_s, (40, 45, 55))
            btn_flip = Button(sidebar_x+20, by+100, 145, 40, "FLIP BOARD", f_s)
            btn_copy = Button(sidebar_x+175, by+100, 145, 40, "COPY LOG", f_s)
            btn_exit = Button(sidebar_x+20, by+150, 300, 45, "NEW GAME" if game.winner else "EXIT TO MENU", f_m)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    # Handle Draw Agreement Checkboxes
                    if not game.winner:
                        if cb_w.collidepoint(event.pos):
                            game.draw_agreement[W_PAWN] = not game.draw_agreement[W_PAWN]
                        if cb_b.collidepoint(event.pos):
                            game.draw_agreement[B_PAWN] = not game.draw_agreement[B_PAWN]
                        # Check if both agreed
                        if game.draw_agreement[W_PAWN] and game.draw_agreement[B_PAWN]:
                            game.winner = "Draw"
                            game.win_reason = "Mutual Agreement"
                    ...
                    if OFFSET < event.pos[0] < BOARD_SIZE + OFFSET and OFFSET < event.pos[1] < BOARD_SIZE + OFFSET and not game.winner:
                        ...
                        if not move_found:
                            if game.board[r][c] == game.current_player:
                                sel_pos, valid_moves = (r, c), game.get_legal_moves(r, c)
            ...
            if confirm_resign_who or confirm_exit_to_menu:
                ...
                screen.blit(f_m.render(msg, True, TEXT_COLOR), (WIDTH//2-f_m.size(msg)[0]//2, HEIGHT//2-30))
                pygame.draw.rect(screen, (60, 140, 80), (WIDTH//2-110, HEIGHT//2+20, 100, 40), border_radius=6)
                pygame.draw.rect(screen, (180, 60, 80), (WIDTH//2+10, HEIGHT//2+20, 100, 40), border_radius=6)
                screen.blit(f_s.render("YES", True, TEXT_COLOR), (WIDTH//2-75, HEIGHT//2+30))
                screen.blit(f_s.render("NO", True, TEXT_COLOR), (WIDTH//2+50, HEIGHT//2+30))
